...
#### MOVING DIRECTION: ordinal logistic regression ####
...
move <- readRDS('../data_processed/behaviour_by_second_indexvariables.RDS') %>% 
  ...
  mutate(move_index = as.integer(factor(direction_move,
                                        levels = c('approach directly','approach at an angle',
                                                   'move directly with',
...
move_no_na <- move_no_na %>% 
  mutate(move_tminus1_num = ifelse(move_tminus1 == 'approach directly', 1,
                                   ifelse(move_tminus1 == 'approach at an angle', 2, 
                                          ifelse(move_tminus1 == 'move directly with', 3,
                                                 ifelse(move_tminus1 == 'move away at an angle', 4,
                                                        ifelse(move_tminus1 == 'move away directly', 5,
                                                               NA)))))
         ) %>% 
...
get_prior(formula = moving_direction ~ 1 + mo(focal_age) * mo(partner_age) + stim_type +   # fixed effects
            time_since_stim + mo(move_tminus1_num) +                                       # controls
...
direction_move_prior <- brm(
  formula = moving_direction ~ 1 + mo(focal_age) * mo(partner_age) + stim_type + 
    time_since_stim + mo(move_tminus1_num) +
