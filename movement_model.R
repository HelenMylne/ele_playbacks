...
# write_csv(move, '../data_processed/behaviour_by_second_indexvariables_move.RDS')
# 
...
## plot overall
ggplot(move_no_na, aes(x = f_age_num, y = moving_direction,
...
# colnames(pred_all) <- 1:nrow(move_no_na)
...
#   as.data.frame()
# pred_t2 <- pred_t %>%
#   mutate(data_row = 1:nrow(move_no_na)) %>%
...
# move_no_na$unique_data_combo <- 1:nrow(move_no_na)
# pred_mtx <- posterior_epred(object = move_fit, newdata = move_no_na)
# colnames(pred_mtx) <- move_no_na$unique_data_combo
# save.image('movement_direction/movement_model_predictions.RData')
#
# ## convert predictions to long format data set, using only first 100 values per chain
# #load('movement_direction/movement_model_predictions.RData')
...
#          pred_type = 1) %>%
#   left_join(move_no_na, by = 'unique_data_combo')
...
#
# ## make labels for age of movement in previous second move nice
# prevsec_labels <- c('movement younger at t-1',
#                     'movement same age at t-1',
#                     'movement older at t-1')
...
#     labs(colour = 'predicted age of movement relative to focal:',
#          fill = 'predicted age of movement relative to focal:',
...
# ggsave(plot = last_plot(), file = '../outputs/move_predictions_violin.png',
...
# save.image('movement_direction/movement_model_agecontrasts_epred.RData')
#
# # movement in previous second -- nn ####
