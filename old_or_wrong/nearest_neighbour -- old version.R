...
get_prior(formula = age_diff_num ~ 1 + mo(f_age_num) * mo(p_age_num) + stim_type +   # fixed effects
...
priors <- c(
  ...
  # partner age
  prior(normal(0,0.25),   class = b,    coef = mop_age_num),
  prior(dirichlet(2,2,2), class = simo, coef = mop_age_num1),
  # interaction age
  prior(normal(0,0.5),    class = b,    coef = mof_age_num:mop_age_num),
  prior(dirichlet(2),     class = simo, coef = mof_age_num:mop_age_num1),
  prior(dirichlet(2),     class = simo, coef =mof_age_num:mop_age_num2),
  # stimulus type
...
num_iter <- 2000
nn_prior <- brm(
  formula = age_diff_num ~ 1 + mo(f_age_num) * mo(p_age_num) + stim_type + 
  ...
  sample_prior = 'only')
pp_check(nn_prior) # prior expects 1 and 3 most likely, 2 least likely. data show all very similar
...
# save workspace
save.image('neighbour_model_run.RData')
#load('neighbour_model_run.RData') ; rm(biologylibs, homedrive, homelibs, homelibsprofile,rlibs,Rversion)

## check outputs ####
library(LaplacesDemon)
#load('neighbour_model_run.RData') # rm(biologylibs, homedrive, homelibs, homelibsprofile, rlibs, Rversion) ; gc()
...
marg <- conditional_effects(nn_fit,
                            effects = c('f_age_num','p_age_num','stim_type',
...
conditional_effects(nn_fit, effects = 'f_age_num',
                    categorical = TRUE,
                    spaghetti = TRUE,
                    #conditions = c('p_age_num','stim_type'),
                    method = 'posterior_epred')
...
conditional_effects(nn_fit, effects = 'p_age_num',
                    categorical = TRUE,
                    spaghetti = TRUE,
                    method = 'posterior_epred')
(age_part_plot <- ggplot(p_age_effect)+
    geom_ribbon(aes(x = p_age_num, ymax = upper__, ymin = lower__, fill = cats__), alpha = 0.4)+
    geom_line(aes(x = p_age_num, y = estimate__, colour = cats__), linewidth = 1)+
...
ggsave(plot = age_part_plot, filename = '../outputs/nn_marginaleffects_agepartner.png', device = 'png',
       width = 8.3, height = 5.8)

conditional_effects(nn_fit, 'stim_type', categorical = TRUE,
                    method = 'posterior_epred')
...
ggplot(nn_no_na, aes(x = f_age_num, y = age_diff_num,
                     colour = as.factor(p_age_num)))+
  geom_jitter(alpha = 0.1)+
  facet_wrap(. ~ stim_type)+
  scale_y_continuous(name = 'age difference category',
                     breaks = c(1,2,3),
                     labels = c('partner younger','age matched','partner older'))+
  labs(colour = 'partner age')
...
pred_all <- array(data = NA, dim = c(nrow(pred), ncol(pred), length(age_types)),
                  dimnames = list(rownames(pred), colnames(pred),
                                  age_types))
pred_all[,,1] <- pred
save.image('neighbour_model_predictions.RData')
...
load('neighbour_model_predictions.RData')
