...
data {
  ...
  int Xmo_4[N];               // monotonic variable
  vector[Jmo[4]] con_simo_4;  // prior concentration of monotonic simplex
...
parameters {
  ...
  simplex[Jmo[4]] simo_4;     // monotonic simplex
...
model {
  ...
  if (!prior_only) {
    ...
    for (n in 1:N) {
      ...
      mu[n] += (bsp[1]) * mo(simo_1, Xmo_1[n]) + (bsp[2]) * mo(simo_2, Xmo_2[n]) + (bsp[3]) * mo(simo_3, Xmo_3[n]) + (bsp[4]) * mo(simo_4, Xmo_4[n]) + r_1_1[J_1[n]] * Z_1_1[n] + r_2_1[J_2[n]] * Z_2_1[n] + r_3_1[J_3[n]] * Z_3_1[n];
    }
    for (n in 1:N) {
